...
// A custom field's value is either a plain string or, for a multi-select dropdown, an
// array of option values. Returns null for anything empty so the caller can drop it.
...
//   customFields — a flat { key: value } map of GHL custom fields, where a value is a
//            string or an array of options for a multi-select. Empty values are
//            dropped so a blank form answer never overwrites what's already on the
//            contact; the rest go up as GHL's [{ key, field_value }] array.
export async function upsertLead(lead, opts = {}) {
  const {
    source = "website chat",
    tags: baseTags = ["chat-lead"],
    customFields = {},
  } = opts;
  ...
  // GHL shows custom field keys with a "contact." prefix, but the upsert endpoint wants
  // the bare key — strip it so a key copied straight out of the GHL UI still works.
  const fields = Object.entries(customFields)
    .map(([key, value]) => ({
      key: key.replace(/^contact\./, ""),
      field_value: normalizeFieldValue(value),
    }))
    .filter((f) => f.field_value !== null);
